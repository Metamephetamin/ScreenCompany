...
Откройте `http://localhost:3000` и создайте аккаунт на странице `/login`.
...
- `7704217370` - ООО "ИНТЕРНЕТ РЕШЕНИЯ"
- `7736207543` - ООО "ЯНДЕКС"
- `7721503606` - компания с исполнительными производствами в DaMIA-ФССП
...
В production неподключенные источники не имитируются: в отчете отображается статус `Источник не подключен`.
В development приложение может использовать mock provider, чтобы локальная разработка не зависела от внешних сервисов.
...
Сейчас без отдельных API-договоров остаются неподключенными:

- судебные дела;
- финансовая отчетность;
- банкротные события.

Оплата тарифов временно работает как заявка на счет. Для онлайн-оплаты планируется CloudPayments.

Авторизация, регистрация, database sessions, валидация форм, риск-скоринг, история, отчеты, сохранение отчета в PDF через печать браузера, мониторинг и UI-сценарии работают.
